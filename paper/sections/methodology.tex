...
(\texttt{src/\allowbreak{}spike\_\allowbreak{}tpu\_\allowbreak{}forward\_\allowbreak{}pass.py:49-57}). AlphaFold2's source itself is not modified.
...
\begin{itemize}
  \item \textbf{Model config:} \texttt{model\_3} by default; \texttt{model\_4} and \texttt{model\_5} only in the
    ...
    (\texttt{src/\allowbreak{}spike\_\allowbreak{}tpu\_\allowbreak{}forward\_\allowbreak{}pass.py:159-162}).
  \item \textbf{Ensembling:} \texttt{num\_ensemble = 1} in every script (e.g.\
    \texttt{src/\allowbreak{}spike\_\allowbreak{}tpu\_\allowbreak{}forward\_\allowbreak{}pass.py:165}). The ensemble experiment builds its 8
  ...
  \item \textbf{Precision:} float32 by default. The bfloat16 run casts every float32
    parameter leaf after \texttt{init\_params} (\texttt{src/\allowbreak{}spike\_\allowbreak{}tpu\_\allowbreak{}forward\_\allowbreak{}pass.py:184-189}).
...
\begin{itemize}
  \item \textbf{118 residues} in the single-query scripts: a fixed toy sequence
    (\texttt{TOY\_SEQUENCE\_118}, \texttt{src/\allowbreak{}spike\_\allowbreak{}tpu\_\allowbreak{}forward\_\allowbreak{}pass.py:107-110}), also used by the
    ensemble and auto-mesh scripts and by the AF3 input (\texttt{src/\allowbreak{}make\_\allowbreak{}af3\_\allowbreak{}input.py:30}).
    It is synthetic, not a real protein.
  \item \textbf{Any other length:} the 20-letter alphabet \texttt{ACDEFGHIKLMNPQRSTVWY} repeated
    and truncated (\texttt{src/\allowbreak{}spike\_\allowbreak{}tpu\_\allowbreak{}forward\_\allowbreak{}pass.py:113-117}).
    \begin{itemize}
...
\begin{itemize}
  \item \textbf{MSA:} a single-sequence MSA containing only the query, built with
    ...
    \texttt{src/\allowbreak{}spike\_\allowbreak{}tpu\_\allowbreak{}forward\_\allowbreak{}pass.py:127-134}). There is no genetic database search
  ...
  \item \textbf{Feature processing:} \texttt{af\_\allowbreak{}features.np\_\allowbreak{}example\_\allowbreak{}to\_\allowbreak{}features(..., random\_\allowbreak{}seed=0)}
    (\texttt{src/\allowbreak{}spike\_\allowbreak{}tpu\_\allowbreak{}forward\_\allowbreak{}pass.py:171-173}). This step runs before any timer
...
\midrule
Feature seed & 0 (ensemble: member index \texttt{i}, 0--7) & \texttt{src/\allowbreak{}spike\_\allowbreak{}tpu\_\allowbreak{}forward\_\allowbreak{}pass.py:172}; \texttt{src/\allowbreak{}spike\_\allowbreak{}ensemble\_\allowbreak{}shard\_\allowbreak{}forward\_\allowbreak{}pass.py:110} \\
Parameter-init seed & 0 & \texttt{src/\allowbreak{}spike\_\allowbreak{}tpu\_\allowbreak{}forward\_\allowbreak{}pass.py:180} \\
\texttt{predict} seed / \texttt{apply} PRNG key & 0 / \texttt{PRNGKey(0)} & \texttt{src/\allowbreak{}spike\_\allowbreak{}tpu\_\allowbreak{}forward\_\allowbreak{}pass.py:198,211}; \texttt{src/\allowbreak{}spike\_\allowbreak{}pmap\_\allowbreak{}forward\_\allowbreak{}pass.py:100} \\
Compilation cache & \texttt{jax\_\allowbreak{}compilation\_\allowbreak{}cache\_\allowbreak{}dir=/\allowbreak{}tmp/\allowbreak{}jax\_\allowbreak{}cache}, min entry size $-1$, min compile time 0; two separate processes & \texttt{src/\allowbreak{}spike\_\allowbreak{}tpu\_\allowbreak{}forward\_\allowbreak{}pass.py:138-142}; \texttt{configs/\allowbreak{}af\_\allowbreak{}spike\_\allowbreak{}combined.yaml:57-64} \\
Auto mesh & \texttt{jax.make\_\allowbreak{}mesh((8, 1), ("fsdp", "tp"))}, both axes \texttt{AxisType.Auto}, under \texttt{jax.set\_mesh} & \texttt{src/\allowbreak{}spike\_\allowbreak{}meshshard\_\allowbreak{}forward\_\allowbreak{}pass.py:90-91} \\
...
\texttt{pmap} + \texttt{pmean} ensemble & \texttt{jax.pmap} over members, \texttt{jax.lax.pmean} of \texttt{predicted\_lddt} logits on axis \texttt{ensemble} & \texttt{src/\allowbreak{}spike\_\allowbreak{}ensemble\_\allowbreak{}shard\_\allowbreak{}forward\_\allowbreak{}pass.py:129,132} \\
...
parameters randomly (\texttt{src/\allowbreak{}spike\_\allowbreak{}tpu\_\allowbreak{}forward\_\allowbreak{}pass.py:175-176}; log line
...
\midrule
\texttt{init\_params} & \texttt{runner.init\_\allowbreak{}params(...)}: Haiku \texttt{init}, including its own JIT trace, compilation and execution & none explicit & \texttt{src/\allowbreak{}spike\_\allowbreak{}tpu\_\allowbreak{}forward\_\allowbreak{}pass.py:178-182} \\
1st \texttt{predict} (compile + run) & \texttt{runner.predict(...)}, \textbf{inside \texttt{jax.profiler.trace(...)}} in \texttt{spike\_\allowbreak{}tpu\_\allowbreak{}forward\_\allowbreak{}pass.py}, without a profiler in the auto-mesh script & \texttt{jax.block\_\allowbreak{}until\_\allowbreak{}ready(result)} & \texttt{src/\allowbreak{}spike\_\allowbreak{}tpu\_\allowbreak{}forward\_\allowbreak{}pass.py:191-201}; \texttt{src/\allowbreak{}spike\_\allowbreak{}meshshard\_\allowbreak{}forward\_\allowbreak{}pass.py:113-118} \\
2nd \texttt{predict} (steady state) & the identical call again, no profiler & \texttt{jax.block\_\allowbreak{}until\_\allowbreak{}ready(result2)} & \texttt{src/\allowbreak{}spike\_\allowbreak{}tpu\_\allowbreak{}forward\_\allowbreak{}pass.py:203-216} \\
\bottomrule
\end{tabularx}
\end{center}

\emph{Note on the script version.} The rows above and the bullets below describe the script
as it produced the August CPU/GPU and the TPU results. Since commit \texttt{86fca03},
\texttt{src/\allowbreak{}spike\_\allowbreak{}tpu\_\allowbreak{}forward\_\allowbreak{}pass.py} makes the first-call profiler optional
(\texttt{-{}-profile\_\allowbreak{}first\_\allowbreak{}predict}, default \texttt{True}) and repeats the steady-state call
\texttt{-{}-num\_\allowbreak{}steady\_\allowbreak{}state\_\allowbreak{}runs} times (default \texttt{1}), which is why the cited lines differ from
the description. The September reruns in \texttt{results/\allowbreak{}repro/} time 5 steady-state calls
with the profiler off (\texttt{-{}-noprofile\_\allowbreak{}first\_\allowbreak{}predict -{}-num\_\allowbreak{}steady\_\allowbreak{}state\_\allowbreak{}runs=5}); each
...
\begin{itemize}
  ...
  \item \textbf{Rounding in JSON:} 2 decimals for \texttt{init\_params} and first predict, 3 for
    steady state (\texttt{src/\allowbreak{}spike\_\allowbreak{}tpu\_\allowbreak{}forward\_\allowbreak{}pass.py:250-252}).
...
\paragraph{Version caveat for the CPU/GPU baselines.} The script in \texttt{src/} and all original
results were first committed together in \texttt{2755250}, so git cannot show which script
version produced which of those results. The script was later changed in \texttt{86fca03}
(optional profiler, repeated steady-state calls); that later version, unchanged in
\texttt{9e1a07e}, is the one the September reruns (\texttt{results/\allowbreak{}repro/}) used.
\begin{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Non-comparable first calls across scripts.} The \texttt{pmap}, ensemble and auto-mesh
  ...
  \item \textbf{Alternative explanation for the auto-mesh first call.} \texttt{results/\allowbreak{}sweep/\allowbreak{}sharding.md} attributes
    ...
    launches the \texttt{pmap} and auto-mesh scripts as separate \texttt{python3} processes, without
...
\texttt{spike\_\allowbreak{}ensemble\_\allowbreak{}shard\_\allowbreak{}forward\_\allowbreak{}pass.py} & yes & yes, \texttt{block\_until\_ready} & no & none (one query) & \texttt{src/\allowbreak{}spike\_\allowbreak{}ensemble\_\allowbreak{}shard\_\allowbreak{}forward\_\allowbreak{}pass.py:135-147} \\
\bottomrule
\end{tabularx}
\end{center}
...
\texttt{src/\allowbreak{}spike\_\allowbreak{}tpu\_\allowbreak{}forward\_\allowbreak{}pass.py:222-234}).
\begin{itemize}
...
\begin{itemize}
  \item \textbf{Per-run JSONs.} The scripts write one \texttt{result\_\textless{}run\_id\textgreater{}.json} per process
    (e.g.\ \texttt{src/\allowbreak{}spike\_\allowbreak{}tpu\_\allowbreak{}forward\_\allowbreak{}pass.py:263-265}). Apart from the three baseline
...
\begin{enumerate}
  ...
  \item How the second auto-mesh run was launched.
  \item Which partitioner actually executed the auto-mesh runs. The Job pins \texttt{jax[tpu]==0.10.2}, which defaults to Shardy, and sets no partitioner flag; the \texttt{"GSPMD"} string in \texttt{results/\allowbreak{}sweep/\allowbreak{}sharding.json}'s \texttt{description} field is hand-written, not recorded. \TODO{NOT IN REPO}
