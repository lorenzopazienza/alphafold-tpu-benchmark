...
function ConfidenceChart() {
  return (
    <div className="h-[min(36vh,280px)] w-full min-h-[200px]">
...
function Legend() {
  ...
  return (
    <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-[12px] text-mute">
      <span className="font-medium text-ink">pLDDT</span>
      {stops.map((s) => (
    ...
    <section id="structure" className="border-t border-line bg-panel">
      <div className="viewport-tight shell">
        <div className="mb-8 flex flex-col gap-3 md:mb-10 md:flex-row md:items-end md:justify-between">
          ...
          <p className="max-w-md text-[15px] leading-relaxed text-slate md:text-right">
        ...
        <div className="grid items-stretch gap-10 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)] lg:gap-14">
          <figure className="relative">
            <div className="relative aspect-square w-full overflow-hidden bg-paper sm:aspect-[5/4] lg:aspect-square lg:min-h-[min(52vh,520px)]">
              <div
                ref={hostRef}
            ...
            <figcaption className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <Legend />
              <p className="text-[12px] text-mute sm:text-right">
                Lower confidence at the C-terminus (residues ~71–76)
              </p>
            </figcaption>
          </figure>

          <figure className="flex flex-col justify-center border-t border-line pt-8 lg:border-l lg:border-t-0 lg:pl-10 lg:pt-0">
            <div className="flex items-baseline justify-between gap-4">
              <h3 className="font-display text-lg font-semibold tracking-tight text-ink">
                Per-residue pLDDT
              </h3>
              <p className="eq font-mono text-[11px] text-mute">
                mean {MEAN_PLDDT}
              </p>
            </div>
